...

{
        ...
        { dist_root => 't/does-not-exist' },
        {
            add_files => {
                path(qw(source dist.ini)) => dist_ini(
                    ...
                    [ GatherDir => ],
                    [ MetaConfig => ],
                    [ 'Git::Contributors' => { mailmap => '.other-mailmap' } ],
                ),
                path(qw(source lib Foo.pm)) => "package Foo;\n1;\n",
                path(qw(source .mailmap)) => "Anon Nonny Moose <anonnonny\@moose.org> Anon Y. Moose <anon\@null.com>\n",
                path(qw(source .other-mailmap)) => "Herp Derp <herp\@derp.org> <jude\@example.org>\n",
            },
        },
    );
    ...
    cmp_deeply(
        ...
        'contributor names are extracted, with additional mailmap file applied',
    ) or diag 'got distmeta: ', explain $tzil->distmeta;
